...
export const updateBlog = async (req, res, next) =>
{
    ...
    if (!blog) {return res.status(500).json({message:"could not update blog"})
    };
    res.status(200).json({blog});
}

export const  getById = async (req, res, next) => {
    const blogIdd = req.params.id
    let blog;
  try {
    blog =  await Blog.findById(blogIdd)
  } 
  catch (err) {return console.log(err)}
   
  if (!blog) { return res.status(404).json({message:"input a valid id"})
  };
